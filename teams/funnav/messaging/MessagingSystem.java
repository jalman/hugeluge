...
public class MessagingSystem {
  ...
  public enum MessageType {
    ...
    SOLDIER_ID(1),
    ENEMY_BOT(2),
    // TODO (damien) figure out variable length messages?
    MAP_FRAGMENT(1)
    ;
  ...
  public void writeMapFragments(int[] frags) throws GameActionException {
    for(int i: frags) {
      // TODO don't repeatedly send headers...
      writeMessage(MessageType.MAP_FRAGMENT, frags[i]);
    }
  }
  ...
  */
  ...
  public void writeMapEdges(int xMin, int xMax, int yMin, int yMax) throws GameActionException {
    writeReservedMessage(ReservedMessageType.MAP_EDGES, (xMin << shift) + xMax, (yMin << shift) + yMax);
  }
  
  public int[] readMapEdges() throws GameActionException {
    int channel = ReservedMessageType.MAP_EDGES.channel();
    int x = RC.readBroadcast(channel), y = RC.readBroadcast(channel+1);
    return new int[]{ x >> shift, x & mask, y >> shift, y & mask };
  }
